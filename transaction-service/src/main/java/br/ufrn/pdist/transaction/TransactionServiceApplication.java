...
import br.ufrn.pdist.transaction.application.AccountLockManager;
import br.ufrn.pdist.transaction.application.LocalAccountLockManager;
import br.ufrn.pdist.transaction.application.RedisAccountLockManager;
...
public class TransactionServiceApplication {
    ...
    CommandLineRunner transactionStartupRunner() {
        return args -> {
            ...
            AccountLockManager lockManager = redisConfig.enabled()
                    ? new RedisAccountLockManager(redisConfig)
                    : new LocalAccountLockManager();
            TransactionRequestHandler handler = new TransactionRequestHandler(transport, gatewayTarget.instance(), lockManager);
